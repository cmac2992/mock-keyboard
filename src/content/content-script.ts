...
interface KeyboardDebugSnapshot {
...
interface OriginalStyleSnapshot {
  transform: string;
  transition: string;
  bottom: string;
}
...
interface RootStyleSnapshot {
  ...
  htmlScrollPaddingBottom: string;
}
...
class MockKeyboardController {
  ...
  private fallbackRafIds: number[] = [];
  private rootStyleSnapshot: RootStyleSnapshot | null = null;
  private shiftedElements = new Set<HTMLElement>();
  private shiftedSnapshots = new WeakMap<HTMLElement, OriginalStyleSnapshot>();
  private autoAnchoredCandidates = new Set<HTMLElement>();
  private candidateCacheDirty = true;
  private candidateObserver: MutationObserver | null = null;
  private debugFocused = new Set<HTMLElement>();
  private debugShifted = new Set<HTMLElement>();
  private debugStyle: HTMLStyleElement | null = null;
  private bridgeReady = false;
  private pendingBridgeDetail: MockKeyboardChangeDetail | null = null;
  private lastDispatchedDetail: MockKeyboardChangeDetail | null = null;
  private lastAutoScrollApplied = false;
  private lastBodyPaddingApplied = false;
  ...
  private render(previous: KeyboardState, source: KeyboardSource): void {
    ...
    if (this.overlay) {
      ...
      this.overlay.safeArea.setAttribute(
        'data-visible',
        String(this.state.enabled && this.state.visible && this.state.debug)
      );
  ...
  private applyGlobalCompensation(): void {
    if (!document.body) {
      return;
    }
  ...
  private restoreGlobalCompensation(): void {
    if (!document.body) {
      return;
    }
  ...
  private scheduleFallbackPass(): void {
    this.cancelFallbackPass();
  ...
  private cancelFallbackPass(): void {
  ...
  private shiftBottomAnchoredElements(): void {
    this.restoreShiftedElements();
  ...
  private restoreShiftedElements(): void {
  ...
  private dispatchChange(source: KeyboardSource): void {
    ...
    const viewport = getViewportMetrics();
    const detail: MockKeyboardChangeDetail = {
    ...
    if (document.documentElement.getAttribute(PAGE_BRIDGE_READY_ATTRIBUTE) === 'ready') {
      ...
      if (this.pendingBridgeDetail) {
        document.dispatchEvent(
          new CustomEvent(CONTENT_BRIDGE_EVENT_NAME, { detail: this.pendingBridgeDetail })
        );
  ...
  private getDebugSnapshot(): KeyboardDebugSnapshot {
    ...
    return {
      bridgeInjected:
        pageWindow.__mockKeyboardBridgeInjected__ ||
  ...
  private ensureCandidateObserver(): void {
  ...
  private disconnectCandidateObserver(): void {
  ...
  private refreshAnchoredCandidates(): void {
    this.autoAnchoredCandidates.clear();

    const bodyElements = document.querySelectorAll<HTMLElement>('body *');
    for (const element of bodyElements) {
      this.autoAnchoredCandidates.add(element);
    }

    const explicitAnchors = document.querySelectorAll<HTMLElement>(`[${ANCHOR_ATTRIBUTE}="bottom"]`);
  ...
  private sourceForCurrentMode(): KeyboardSource {
    if (!this.state.enabled) {
      return 'disabled';
    }
...
function computeKeyboardHeight(preset: KeyboardPreset, viewportHeight: number): number {
...
function readPageDebugSnapshot(): PageDebugSnapshot | null {
...
function isBottomAnchoredCandidate(
  element: HTMLElement,
...
function getFocusedElementOverlap(element: HTMLElement, keyboardHeight: number): number {
  const viewport = getViewportMetrics();
  const rect = element.getBoundingClientRect();
...
function makeStyleSnapshot(element: HTMLElement): OriginalStyleSnapshot {
  return {
...
function mergeTransform(existingTransform: string, keyboardHeight: number): string {
...
function setCssVar(style: CSSStyleDeclaration, name: string, value: string): void {
...
function buildKeyboardMarkup(): string {
  ...
  const lk = (ch: string) => `<span class="mk-key mk-key--letter">${ch}</span>`;
  const fk = (label: string) => `<span class="mk-key mk-key--fn">${label}</span>`;
